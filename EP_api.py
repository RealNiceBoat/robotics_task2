...
# ROBOT COMMANDS
	...
	def lights(self, inputstring): # NEW: example command to change LED color. see usage in the 'scan' method
		if inputstring == 'red': # lights up red for 1s and change to white
			self._sendcommand('led control comp bottom_all r 255 g 0 b 0 effect solid')
			time.sleep(1)
			self._sendcommand('led control comp bottom_all r 255 g 255 b 255 effect solid')
		if inputstring == 'green': # lights up green for 1s and change to white 
			self._sendcommand('led control comp bottom_all r 0 g 255 b 0 effect solid')
			time.sleep(1)
			self._sendcommand('led control comp bottom_all r 255 g 255 b 255 effect solid')
	
	def scan(self): # NEW: example command to scan 3 positions based on fixed angles. you should be creating your own logic using this as a baseline
		self.move('z -90 vz 10')
		time.sleep(9)
		self.move('z 45 vz 10') # first example position
		time.sleep(5)
		self.lights('red') # assuming false 
		self.move('z 45 vz 10') # second example position
		time.sleep(5)
		self.lights('green') # assuming true
		self.move('z 45 vz 10') # third example position
		time.sleep(5)
		self.lights('red') # assuming false
		self.move('z 45 vz 10')
		time.sleep(5)
